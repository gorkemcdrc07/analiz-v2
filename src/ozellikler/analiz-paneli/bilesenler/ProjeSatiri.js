...
import {
    Box, Collapse, Stack, Typography, Tooltip,
} from "@mui/material";
import { useTheme } from "@mui/material/styles";
import {
    MdKeyboardArrowDown, MdLocalShipping,
...
import { TbClockHour4, TbFileInvoice } from "react-icons/tb";
            ...
            {copyable && !empty && (
                <Box
                    ...
                    sx={{
                        ...
                        color: copied
                            ? "#34D399"
                            : (isDark ? "rgba(160,170,200,0.4)" : "rgba(80,90,120,0.4)"),
                ...
                <Box sx={{ p: 0.75 }}>
                    ...
                    <Box>
                        <DataField
                            ...
                            label="Ruhsat Sahibi"
                            value={v(safe.VehicleCurrentAccountTitle)}
                            isDark={isDark}
                            accentColor="#818CF8"
                        />
                        <DataField
...
function SeferKarti({
    ...
    const isLate = zaman.label === "Yüklemeye Geciken";

    const LATE_RED = "#F87171";
    const borderAcc = isLate
        ? LATE_RED
                                ...
                                rows={[
                                    {
                                        ...
                                        value: tarihFormatla(item?.PickupDate),
                                        color: "#818CF8",
                                    },
                                    {
                                        icon: <TbClockHour4 size={14} />,
                                        label: "Noktaya Geliş",
                                        value: tarihFormatla(loadingDate),
                                        color: "#34D399",
                                    },
                                    {
                                        icon: <MdOutlineTimer size={14} />,
                                        label: "Tahmini Varış Zamanı",
                                        value: tarihFormatla(item?.EstimatedArrivalTime),
                                        color: "#FBBF24",
                                    },
                                    {
                                ...
                                rows={[
                                    {
                                        ...
                                        label: "Sefer Açılış",
                                        value: tarihFormatla(item.TMSDespatchCreatedDate),
                                        color: "#A78BFA",
                                    },
                                    {
                                        icon: <TbFileInvoice size={14} />,
                                        label: "Sipariş",
                                        value: tarihFormatla(item.OrderCreatedDate),
                                    },
                                    {
                                        icon: <MdPerson size={14} />,
                                        label: "Operasyon",
                                        value: item.OrderCreatedBy || "-",
                                    },
                                ]}
                            />
                        </Stack>
...
export default function ProjeSatiri({
    ...
    const altDetaylar = useMemo(
        () => altDetaylariOlustur(satir.name, tumVeri),
        [satir.name, tumVeri]
    );
    ...
    const talep = useMemo(
        () => Math.max(0, Number(satir?.plan ?? 0) || 0),
        [satir?.plan]
    );

    const tedarikEdilen = useMemo(
        () => Math.max(0, Number(satir?.ted ?? 0) || 0),
        [satir?.ted]
    );

    const tedarikEdilmeyen = useMemo(
        () => Math.max(0, talep - tedarikEdilen),
        [talep, tedarikEdilen]
    );

    // YENİ MANTIK:
    // Zamanında adet = tedarik edilen - geç tedarik
    // Zamanında yüzde = zamanında adet / toplam talep
    const zamanindaAdet = useMemo(
        () => Math.max(0, tedarikEdilen - gecTedarikSayisi),
    ...
    const zamanindaYuzde = useMemo(() => {
        ...
        return Math.max(
            0,
            Math.min(100, Math.round((zamanindaAdet / talep) * 100))
        );
    }, [talep, zamanindaAdet]);

    const zamanindaRenk = useMemo(
        () => zamanindaRenkFromPct(zamanindaYuzde),
        [zamanindaYuzde]
    );
    ...
    const siraliAltDetaylar = useMemo(() => {
        ...
        list.sort((a, b) => {
            ...
            return (
                (parseTRDateTime(b?.EstimatedArrivalTime)?.getTime?.() ?? 0) -
                (parseTRDateTime(a?.EstimatedArrivalTime)?.getTime?.() ?? 0)
            );
        });
                            ...
                            >
                                ...
                                >
                                    <Box sx={{ display: "flex", gap: 0.5, alignItems: "center" }}>
                                        ...
                                        <Typography sx={{ fontSize: "0.75rem", fontWeight: 900, color: "#60A5FA" }}>
                                            {satir.spot}
                                        </Typography>
                                    </Box>
                                    ...
                                    <Box sx={{ display: "flex", gap: 0.5, alignItems: "center" }}>
                                        ...
                                        <Typography sx={{ fontSize: "0.75rem", fontWeight: 900, color: "#A78BFA" }}>
                                            {satir.filo}
                                        </Typography>
                                    </Box>
                                    ...
                                    <Box sx={{ display: "flex", gap: 0.5, alignItems: "center" }}>
                                        ...
                                        <Typography sx={{ fontSize: "0.75rem", fontWeight: 900, color: "#34D399" }}>
                                            {satir.sho_b}
                                        </Typography>
                                    </Box>
                                </Box>
                                ...
                                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                    <Tooltip
                                        title={`Zamanında Oran: %${zamanindaYuzde} (${zamanindaAdet}/${talep})`}
                                    >
                                        <Box
                                            sx={{
                                                display: "flex",
                                        ...
                                        >
                                            ...
                                            <Typography
                                                sx={{
                                                    fontSize: "0.52rem",
                                                    fontWeight: 700,
                                                    color: isDark ? "rgba(160,170,200,0.35)" : "rgba(80,90,120,0.35)",
                    ...
                    >
                        <Box
                            sx={{
                                ...
                                border: `0.5px solid ${acik
                                    ? `${zamanindaRenk}30`
                                    : (isDark ? "rgba(255,255,255,0.09)" : "rgba(0,0,0,0.08)")
                                    }`,
                                color: acik
                                    ? zamanindaRenk
                                    : (isDark ? "rgba(160,170,200,0.5)" : "rgba(80,90,120,0.5)"),
                                transition: "all 0.2s",
                            }}
                        >
